...
\todo{show refined sum program in llM monad}
...
\begin{gather*}
  \{(\iexp{ls\_assn} \app \iexp{si8\_assn}) \app xs \app xsi \app \ast \uparrow((\iexp{sum'} \app (+) \app 0 \app xs) \le 127 \land -128 \le (\iexp{sum'} \app (+) \app 0 \app xs))\} \\
  (\iexp{ll\_foldl} \app \iexp{add} \app \iexp{0x0} \app xsi) \\
  \{\lambda r. \iexp{si8\_assn} \app (\iexp{sum'} \app (+) \app 0 \app xs) \app r \app \ast (\iexp{ls\_assn} \app \iexp{si8\_assn}) \app xs \app xsi\}
\end{gather*}
...
Note that splitting up the lists $\iexp{xs}$ and $\iexp{ys}$ into their head and tail destroys the original lists.
Concretely, the split is implemented by the $\iexp{os\_pop}$ function of the original open list implementation\footnote{cf. theory $\iexp{Isabelle\_LLVM.IICF\_Open\_List}$.} from Isabelle-LLVM for which it holds that
\begin{gather*}
...
\end{gather*}
which implies that the refinement of $\iexp{merge}$ necessarily also destroys the given input lists.
