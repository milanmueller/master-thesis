...
\textcolor{red}{
  Die Subsection hier werde ich nochmal komplett neu schreiben - lohnt sich nicht den Teil hier zu lesen.
  Ab \ref{sec:isabelle-hol} kommt der (halbwegs) fertige Teil.
}

This section introduces the practical algebraic calculus (PAC).
For a thorough introduction on PAC, in particular the underlying concepts and their use in verification of digital circuits, we refer to the work by Kaufmann et al. \cite{pac-2018} \cite{pac-2024} \cite{kaufmann-phd-thesis}.
...
\begin{itemize}[noitemsep, topsep=0pt, parsep=0pt]
  ...
  \item A nonempty subset $I \subseteq \mathbb{K}[X]$ is called \textit{ideal} if it is closed under multiplication and addition, i.e.
...
Unfolding the type of a $\iexp{polynomial}$ gives $(\iexp{char} \app \iexp{list} \app \iexp{list} \times \iexp{int}) \app \iexp{list}$ which in the Pastèque implementation is abbreviated by $\iexp{llist\_polynomial}$.
