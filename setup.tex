...
% Indentation for continuation lines of a broken Hoare-triple: \hcont{<prefix of the first line>}
% indents the continuation to the column right after <prefix>. The trailing {} keeps the spacing
% of a binary operator (e.g. \ast) at the end of <prefix>.
\newcommand{\hcont}[1]{\phantom{#1{}}}
\usepackage{siunitx}
\usepackage{hyperref}
...
\newcommand{\refalgo}[1]{Algorithm~\ref{#1}} % algorithm/listing reference prefixed with "Algorithm"
...
\makeatletter
\def\isacode@stripprefix/isacode/#1\@nil{#1}
\pgfkeys{
  /isacode/.is family,
  /isacode/label/.store in=\isacode@label,
  /isacode/label/.default=,
  /isacode/caption/.store in=\isacode@caption,
  /isacode/caption/.default=,
  /isacode/.unknown/.code={%
    \edef\isacode@keyname{\expandafter\isacode@stripprefix\pgfkeyscurrentkey\@nil}%
    \ifx\isacode@rest\@empty
      \edef\isacode@rest{\isacode@keyname=\pgfkeyscurrentvalue}%
    \else
      \edef\isacode@rest{\isacode@rest,\isacode@keyname=\pgfkeyscurrentvalue}%
    \fi
  },
}
% label/caption are rendered below the box (in a smaller font) instead of
% inside it, since listings' native captionpos=b places the caption inside
% the tcolorbox frame together with the code.
% Isabelle theory files are read from this directory instead of their
% original location: build.sh/watch.sh run
% scripts/prepare-isabelle-listings.sh beforehand, which copies each theory
% file here with "\<comment>" replaced by a literal em dash. listings cannot
% both recognize a token as a comment delimiter (needed for the gray/italic
% comment styling) and substitute its display text via `literate`, so the
% substitution has to happen before the file reaches listings.
\newcommand{\isabellelistingdir}{build/isabelle-listings}
\newcommand{\isabellecodeextern}[2][]{%
  \def\isacode@rest{}%
  \def\isacode@label{}%
  \def\isacode@caption{}%
  \pgfkeys{/isacode,#1}%
  % Force full expansion of \isacode@rest into literal text before
  % \tcbinputlisting grabs its argument: xkeyval's key=value splitting
  % (used internally by \lstset) scans for literal "," and "=" characters
  % and does not expand macros hidden inside an argument to find them.
  \edef\isacode@fullargs{%
    isabellebox,%
    listing file={\isabellelistingdir/#2},%
    listing options={style=isabellestyle,\isacode@rest}%
  }%
  \expandafter\tcbinputlisting\expandafter{\isacode@fullargs}%
  \ifx\isacode@caption\@empty
    \ifx\isacode@label\@empty\else\label{\isacode@label}\fi
  \else
    \vspace{0.5em}%
    % \label must stay inside this group: \captionof sets \@currentlabel
    % locally, and it would revert to a stale value (e.g. the enclosing
    % section number) once the group closes.
    {\footnotesize
      \captionof{lstlisting}{\isacode@caption}%
      \ifx\isacode@label\@empty\else\label{\isacode@label}\fi
    }%
  \fi
}
